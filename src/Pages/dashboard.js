...
const Dashboard = ({ navigation }) => {
  ...
  const getData = async (userId, token) => {
    ...
    for (var i = 0; i < temp.length; i++) {
      ...
      if (temp[i].property_image) {
        tempImage =
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdVES3nzS4rzEX4Mmy9q5yyw_jN8-5HQSO6g&usqp=CAU';
      } else {
  ...
  const statusChange = (id, listing_id, status) => {
    ...
    axios.get('https://backend.virtualsfadmin.com/sanctum/csrf-cookie').then(
      (response) => {
        axios
          .post(
            'https://backend.virtualsfadmin.com/api/listing/update-status',
            {
              status: status,
      ...
      temp.map((t) => {
        if (t.id == listing_id) {
          finalTemp = {
            ...
            available_supply:
              parseInt(t.available_supply) - parseInt(total_supply),
